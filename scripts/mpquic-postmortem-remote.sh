...
client_remote_dir="$(ssh "$CLIENT_HOST" 'ls -1dt /var/log/mpquic-diag-client-* 2>/dev/null | head -n 1' || true)"
server_remote_dir="$(ssh "$SERVER_HOST" 'ls -1dt /var/log/mpquic-diag-server-* 2>/dev/null | head -n 1' || true)"
...
ssh "$CLIENT_HOST" "sudo tar -C / -czf - ${client_remote_dir#/}" > "$RUN_DIR/client.tgz"
ssh "$SERVER_HOST" "sudo tar -C / -czf - ${server_remote_dir#/}" > "$RUN_DIR/server.tgz"
