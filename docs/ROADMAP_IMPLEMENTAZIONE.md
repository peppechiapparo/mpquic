...
## Fase 4c — Throughput Optimization Sprint 🔄 IN CORSO
...
| 4 | **TCP flow parallelismo** | ~20-40 Mbps | Più flussi iperf3 saturano meglio i path |

### Step 4c.1 — Batch TUN reads + buffer pool 🔄 IN CORSO

**Problema**: ogni pacchetto TUN richiede:
1. `tun.Read(buf)` → 1 syscall `read(2)` (~2-5µs)
2. `append([]byte(nil), buf[:n]...)` → 1 heap allocation (~50-200 ns)
3. Dispatch via channel → context switch overhead
...
### Step 4c.2 — Test baseline senza encryption
...
### Step 4c.3 — Ottimizzazione GC/alloc hot path
...
### Step 4c.4 — Benchmark incrementale
