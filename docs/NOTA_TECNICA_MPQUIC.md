...
**Fase 4b.6–4c — Ottimizzazioni Stripe (ARQ, Batch I/O, GSO, XOR FEC)**

18. [Ottimizzazioni Stripe — Riepilogo Risultati](#18-ottimizzazioni-stripe--riepilogo-risultati)
...
## 18. Ottimizzazioni Stripe — Riepilogo Risultati

Le sezioni 18–18g documentano il ciclo di ottimizzazione profiling-driven che ha
portato il throughput da 239 Mbps (baseline FEC adattivo M=0) a **374 Mbps** (media
fair-weather) con picco di **548 Mbps**. Per i dettagli implementativi di ogni step,
vedere `ROADMAP_IMPLEMENTAZIONE.md` (Steps 4b.6 – 4.27).

### Evoluzione prestazionale

| Versione | Step | Ottimizzazione | Media | Picco | CPU server | Commit |
|---|---|---|---|---|---|---|
| v4.0 | 4b.5 | FEC Adattivo M=0 (baseline) | 239 Mbps | 294 Mbps | ~160% | `3867aae` |
| v4.0 | 4b.6 | Hybrid ARQ v1 (NACK selettivo) | 274 Mbps | — | — | `d158b0a` |
| v4.0 | 4b.6 | ARQ v2 (dedup, rate limit 30ms, nackThresh 96) | 330 Mbps | — | ~120% | `9478e56` |
| v4.0 | 4b.6 | + Batch I/O (recvmmsg, 8 dgram/syscall) | 341 Mbps | 390 Mbps | ~120% | `1e9a8b3` |
| **v4.1** | **4b.7** | **Socket Buffers 7 MB + TX ActivePipes cache** | **354 Mbps** | **390 Mbps** | **~144%** | `bef0894` |
| v4.2-pre | 4.20+4.21 | sendmmsg batch TX + tunWriter drain | 296\* | 458 Mbps | 108% | — |
| **v4.2** | **4.23** | **TUN IFF_MULTI_QUEUE** | **374 Mbps** | **499 Mbps** | **116%** | — |
| v4.4 | 4.24 | UDP GSO client (UDP_SEGMENT) | 364 Mbps | **548 Mbps** | — | — |
...
### Analisi risorse sotto carico (test 6 minuti, 14.3 GB trasferiti a 341 Mbps)

| Risorsa | Client (VM Proxmox) | Server VPS (Linode 8GB) |
|---|---|---|
| **CPU** | ~1.5% (2 vCPU) | ~30% per core (4 vCPU) |
| **RAM** | 3.29 GB / 4.00 GB, stabile | 8 GB, stabile |
| **Packet loss Starlink** | 1% (17/1019 ping) | — |
| **Retransmit TCP** | ~99/s | — |

Il **bottleneck è Starlink** (variabilità beam, handover, weather), non il software.
Efficienza canale: $\eta = 354 / 420 = 84.3\%$ (354 Mbps su 420 Mbps grezzo dual Starlink).

### Bug critico risolto: feedback loop rxDirectCount (XOR FEC adaptive)

Il gate XOR adaptive restava permanentemente ON nonostante loss < 2%: il recovery XOR
non incrementava `rxDirectCount`, causando una sovrastima della loss percepita dal peer
(positive feedback loop). Fix: `atomic.AddUint64(&rxDirectCount, 1)` dopo ogni recovery.
Commit `1596413`.

### Lezione Step 4.27: distribuzione asimmetrica è ottimale

L'esperimento weighted scheduler (3 iterazioni) ha dimostrato che forzare distribuzione
...
obiettivi delle fasi di sviluppo.

### Sintesi risultati per fase

| Fase | Risultato chiave | Metrica |
|------|-----------------|---------|
| **2 — Multi-tunnel** | Isolamento perfetto: 30% loss su un tunnel → 0 impatto sugli altri | RTT Δ = 0 ms, Throughput Δ = ±0% |
| **3 — BBR + Reliable** | Reliable transport 3× throughput base; BBR +79% vs Cubic sotto loss | 15 → 50 Mbps; 26 vs 14 Mbps |
| **4 — Failover/Bonding** | Switchover 2s, 2 pkt persi; bonding 74.3 Mbps; speedtest 72 Mbps | End-to-end trasparente |
| **4b — UDP Stripe** | Bypass Starlink 80 Mbps/session cap; 313 Mbps (+321%) | 12 pipe × 3 WAN |
| **4b.5-4b.7 — FEC/ARQ** | FEC adattivo M=0 + ARQ NACK → 354 Mbps (+48%) | Efficienza 84.3% |
| **4b.8 — Profiling** | sendmmsg + TUN MQ → CPU −28%, picco 499 Mbps | 374 Mbps media |
...
Per i comandi operativi completi (verifica stato, packet loss, iperf3, nftables,
multipath), consultare:
- **INSTALLAZIONE_TEST.md** — procedure e comandi di installazione/test
- **TUNNEL_OPERATIONS_DEBUG.md** — debug per sintomo e raccolta evidenze
