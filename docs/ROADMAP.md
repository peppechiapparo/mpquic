...
## Addendum — Merge v5.2→CAL + ML-KEM reintrodotto e validato sul banco (2026-07-29 sera)

La linea CAL (`ottavionovella`) ora contiene la v5.2 (merge `13d6627`, un conflitto, banco in pari col riferimento) e il post-quantum è garantito e osservabile (`stripe_kex_pq: require`, gauge `mpquic_path_stripe_kex_pq`, sezione `crypto:` → CryptoSession con `hybrid_security` + rekey; commit `ac9bfb4`). Validato su TBOX-EVO: PQ su entrambi i path, decrypt_fail 0 attraverso la finestra di rekey, performance in pari (up 63, down 215-258). Aperti: prova positiva del rekey nel journal/metrica di epoca; attivazione su IBLEA-M (gated). Dettaglio: CHANGELOG 2026-07-29 sera + TDD Rev 5.
...
- **La scoperta che ha cambiato tutto**: i collaudi non attraversavano STRIPES. Un meccanismo di bypass nel policy routing (default attivo `enp7s8` in `MPQUIC_BYPASS_WANS`) faceva uscire il percorso STARLINK sul fisico nudo, e la host route /32 verso l'IP pubblico del VPS dirottava sul fisico anche gli iperf di collaudo. Il bypass è stato RIMOSSO (a tunnel giù solo blackhole; il fallback fisico è ora esplicito e governato da mwan3 su VLAN dedicate 95/96, modello BOND1/VLAN17). In parallelo, hardening del routing: rule statiche di proprietà networkd (immuni allo sweep `ManageForeignRoutingPolicyRules`), pavimenti blackhole su TUTTE le tabelle custom (wanN, phyN, bd1), refresh event-driven + timer di riconciliazione 30s.
- **Il ritorno indietro**: tolto il bypass, il malfunzionamento della versione corrente (CAL/ML-KEM) non era più mascherabile. Decisione: rollback del binario alla baseline pre-CAL `3c03829` su client e VPS lab, e diagnosi vera del trasporto. **La versione con l'adapter per il fornitore NQX e ML-KEM NON è persa**: vive nel branch `feat/crypto-abstraction-layer` e nello snapshot integrale `debug/anomalia-mpquic-20260727` (incluso il WIP non committato), pronta per il debug e la reintroduzione a algoritmo stabilizzato.
- **La causa radice del single-stream, finalmente inchiodata con i numeri**: su un P1 il VPS trasmette 22.771 pacchetti, il client ne riceve 22.770 (perdita 0.004%) ma **22.767 arrivano fuori ordine**: lo striping round-robin per-pacchetto sulle 12 pipe riordina ~il 100% del flusso, TCP lo legge come perdita, l'ARQ tempesta (217 NACK/s, 76% del ricevuto = ritrasmissioni) e lo stimatore segna un dataloss 100% finto. Era il problema di riordino che comprometteva le prestazioni dell'algoritmo da sempre, nascosto dal bypass e dai test sul percorso sbagliato.
- **Il fix: flow-affinity per pipe** (`stripe_flow_affinity`, default off, wire invariato): hash FNV-1a del 5-tuple interno, ogni flusso su una pipe. Forma finale **asimmetrica** decisa dall'A/B: attiva SOLO lato server (downstream, la direzione malata); lato client resta round-robin perché l'affinity in TX uccideva l'upload quando il flusso finiva su una pipe con binding CGNAT degradato (col RR il danno era 1/12, con l'affinity il 100%). Branch `feat/flow-affinity-3c03829`, commit `4c469dc`.
- **Risultati (stessa notte, stesso link, inoltrato da pclab, target = IP di tunnel)**: single-stream download **0.3-21 → 43-85 Mbit**; aggregato P30 **80-122 → 153-187 Mbit**; upload invariato (13-21). Sull'antenna sostituita il 28/07 (loss grezza ~9%): 133-136 Mbit sostenuti senza collassi con il FEC adattivo che recupera il 9.3% dei pacchetti e ARQ residuale all'1.2-1.5% (duplicati ~0.9%): la scala di recuperi disegnata.
- **Regole di validazione nuove** (estendono quelle di TS-016): i test del trasporto SOLO verso gli IP di tunnel (mai l'IP pubblico del VPS); verificare sempre l'IP pubblico di uscita dalla LAN; budget dati stimato e approvato prima di ogni campagna su Starlink a consumo (k6 office 50 VU ≈ 12 GB/h; quota esaurita = ~1 Mbit TCP con ICMP 0%, non un guasto); pavimento fisico nello stesso minuto di ogni sessione.
- **Dual-path VALIDATO (28/07) e versione taggata `v5.1`**: upload bondato 28.9 Mbit, aggregato 213 Mbit a ripartizione capacity-proportional spontanea, P1 33-108 Mbit sul path della pipe assegnata. **Aperti**: tuning ARQ range-retransmit (59% delle retx sono duplicati inutili); debug della versione CAL sul branch e reintroduzione di NQX/ML-KEM sopra l'algoritmo stabilizzato; handoff al team tbox per IBLEA-M sospeso fino al freeze della baseline.
