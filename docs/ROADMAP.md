...
- **Aperti**: validazione dual-path con LTE reale (l'affinity assegna implicitamente i flussi ai path: da vedere con due canali eterogenei PRIMA di taggare una versione); tuning ARQ range-retransmit (59% delle retx sono duplicati inutili); debug della versione CAL sul branch e reintroduzione di NQX/ML-KEM sopra l'algoritmo stabilizzato; handoff al team tbox per IBLEA-M sospeso fino al freeze della baseline.
